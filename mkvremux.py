...
def remux_file(input_file, probe_info, tracks_to_remove, output_dir, new_file_suffix=' (1)', preferences=(('title', False),)):
    """    
    ...
    if tracks_to_remove is not None:
        ...
        for el in tracks_to_remove:
            ...
            mkv_cmd_add.append(['--'+vl+'-tracks', '!'+','.join([str(v) for v in tracks_to_remove[el]])])
...
def main():
    ...
    if inputs['type'] == 'file':
        ...
        raise Exception('Invalid path type `{input_type}`'.format(input_type=inputs['type']))
    ...
    minTracksPerStream, minTracksIdx, maxTracksPerStream, maxTracksIdx = tracksPerStream(track_count)
